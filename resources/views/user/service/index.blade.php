        ...
        @foreach($services as $service)
            <tr @if ($service->status == 1)
                ...
                <td>
                    ...
                    <form action="{{route('services.status', ['id' => $service->id])}}" method="post" style="display: inline-block">
                        {{csrf_field()}}
                        @if (!$service->status)
...
@section('scripts')
    <script>
        $('#modal-delete').on('show.bs.modal', function (event) {
            ...
            $(this).find('.modal-footer form').attr('action', '{{url('services/')}}' + '/' + pid);
        });
        $('#modal-change').on('show.bs.modal', function (event) {
            let pid = $(event.relatedTarget).data('serviceid');
            $(this).find('form').attr('action', '{{url('services/')}}' + '/' + pid + '/password');
        });
    </script>
@endsection